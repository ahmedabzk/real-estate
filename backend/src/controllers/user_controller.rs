...
pub async fn login(
...
) -> Result<Json<UserResponse>, MyError>{
    if payload.username.is_empty() || payload.password.is_empty() {
        return Err(MyError::MissingCredential("missing credentials".to_string()));
    }
    ...
    if let Some(user) = user{
        ...
        if !password_verification{
            return Err(MyError::WrongCredential("check your password and username".to_string()));
        }else{
            ...
            .body(Json(res)).unwrap().into_body();
        ...
        return Err(MyError::UserDoesNotExist("User does not exist".to_string()));
    }
}
